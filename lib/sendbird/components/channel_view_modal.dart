...
import 'package:sign_in_flutter/sendbird/components/user_profile.dart';
...
class ChannelViewModel
  ...
  void dispose() async {
    super.dispose();
    SendbirdSdk().removeChannelEventHandler('channel_listener');
    channel.endTyping();
    isDisposed = true;
  }
  ...
  void showProfile(BuildContext context, Sender sender) async {
    ...
    Navigator.push(context, MaterialPageRoute(builder: (context) => GuestProfile(uuid: sender.userId, bottomCheck: true)));
  }
...
extension Message on BaseMessage {
  bool get isMyMessage => sender?.userId == SendbirdSdk().currentUser?.userId;
}
